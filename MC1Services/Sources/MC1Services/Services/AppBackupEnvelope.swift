...
public struct BackupManifest: Codable, Sendable, Equatable {
  ...
  public init(from envelope: AppBackupEnvelope) {
    deviceCount = envelope.devices.count
    contactCount = envelope.contacts.count
    channelCount = envelope.channels.count
    messageCount = envelope.messages.count
    messageRepeatCount = envelope.messageRepeats.count
    reactionCount = envelope.reactions.count
    roomMessageCount = envelope.roomMessages.count
    remoteNodeSessionCount = envelope.remoteNodeSessions.count
...
public struct ImportResult: Sendable, Equatable {
  ...
  public var totalInserted: Int {
    counts.values.reduce(0) { $0 + $1.inserted }
  }

  public var totalMerged: Int {
    counts.values.reduce(0) { $0 + $1.merged }
  }

  public var totalSkipped: Int {
    counts.values.reduce(0) { $0 + $1.skipped }
  }

  public var totalDropped: Int {
    counts.values.reduce(0) { $0 + $1.dropped }
  }

  public var totalRestoredRecordCount: Int {
    totalInserted + totalMerged
  }
