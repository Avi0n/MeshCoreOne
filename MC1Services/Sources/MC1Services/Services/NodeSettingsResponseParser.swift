...
public enum NodeSettingsResponseParser {
    ...
    public enum SettingsField: Sendable {
        ...
        var query: String {
            switch self {
            case .radio: return "get radio"
            case .txPower: return "get tx"
            case .firmwareVersion: return "ver"
            case .deviceTime: return "clock"
            case .latitude: return "get lat"
            case .longitude: return "get lon"
            case .name: return "get name"
            case .ownerInfo: return "get owner.info"
            }
        }
    }
    ...
    public static func firstSettingsValue(
        ...
        for field in fields {
            ...
            switch (field, parsed) {
            case (.radio, .radio(let frequency, let bandwidth, let spreadingFactor, let codingRate)):
                return .radio(
                    frequency: frequency,
                    bandwidth: bandwidth,
            ...
            case (.txPower, .txPower(let power)):
                return .txPower(power)
            case (.firmwareVersion, .version(let version)):
                return .firmwareVersion(version)
            case (.deviceTime, .deviceTime(let time)):
            ...
            case (.name, .name(let name)):
                return .name(name)
            case (.ownerInfo, .ownerInfo(let info)):
    ...
    public static func behaviorLateResponse(
        ...
        if !hasFloodInterval {
            if case .floodAdvertInterval(let interval) = CLIResponse.parse(
    ...
    public static func isPasswordChangeSuccessful(_ response: String) -> Bool {
        ...
        case .ok:
            return true
        case .raw(let text):
            return text.hasPrefix(passwordChangedPrefix)
        default:
            return false
        }
    }
