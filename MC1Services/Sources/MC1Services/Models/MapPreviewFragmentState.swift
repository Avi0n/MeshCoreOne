...
/// re-read `@Environment(\.colorScheme)`. `isReady` flips true once the snapshot
/// for the current `(rounded lat/lon, isDark)` is resolved (cached or failed),
/// which changes this `Hashable` value and reloads the row.
public struct MapPreviewFragmentState: Sendable, Hashable {
    ...
    public init(latitude: Double, longitude: Double, isDark: Bool, isReady: Bool) {
