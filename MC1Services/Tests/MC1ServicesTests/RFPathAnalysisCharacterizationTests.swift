...
struct RFPathAnalysisCharacterizationTests {
  ...
  @Test
  func `Flat 6km path at 910MHz pins clear-status outputs`() {
  ...
  @Test
  func `100m mountain at 6km pins blocked-status outputs`() throws {
    let result = RFCalculator.analyzePath(
    ...
    #expect(result.distanceMeters == 6000.0)
    #expect(result.freeSpacePathLoss == 107.19385285409474)
    #expect(result.peakDiffractionLoss == 23.034080243532884)
    #expect(result.totalPathLoss == 130.2279330976276)
  ...
  @Test
  func `45m hill with asymmetric heights at 868MHz, k=1.33 pins outputs`() throws {
    let result = RFCalculator.analyzePath(
    ...
    #expect(result.peakDiffractionLoss == 17.211410686784944)
    #expect(result.totalPathLoss == 123.99483019798764)
    #expect(result.clearanceStatus == .blocked)
  ...
  @Test
  func `Rolling 10km terrain pins multi-region outputs`() throws {
    let result = RFCalculator.analyzePath(
      elevationProfile: Self.rollingProfile(totalDistance: 10000, count: 41),
    ...
    #expect(result.distanceMeters == 10000.0)
    #expect(result.freeSpacePathLoss == 111.59256395353627)
    #expect(result.peakDiffractionLoss == 20.388551277139328)
    #expect(result.totalPathLoss == 131.98111523067558)
    #expect(result.clearanceStatus == .blocked)
  ...
  @Test
  func `A-to-R segment over rolling terrain pins outputs`() throws {
    let rolling = Self.rollingProfile(totalDistance: 10000, count: 41)
    ...
    #expect(result.peakDiffractionLoss == 18.929132368423446)
    #expect(result.totalPathLoss == 124.5010964086801)
    #expect(result.clearanceStatus == .blocked)
  ...
  @Test
  func `R-to-B segment keeps absolute obstruction distances`() throws {
    let rolling = Self.rollingProfile(totalDistance: 10000, count: 41)
    let result = RFCalculator.analyzePathSegment(
    ...
    #expect(result.distanceMeters == 5000.0)
    #expect(result.freeSpacePathLoss == 105.57196404025665)
    #expect(result.peakDiffractionLoss == 22.69158176325522)
    #expect(result.totalPathLoss == 128.26354580351187)
    #expect(result.clearanceStatus == .blocked)
  ...
  @Test
  func `Full profile passed as a slice matches analyzePath exactly`() {
    let rolling = Self.rollingProfile(totalDistance: 10000, count: 41)
  ...
  @Test
  func `Degenerate inputs return the zeroed blocked result`() {
