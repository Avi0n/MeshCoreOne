...
extension [MessageStatusEvent] {
  /// Message IDs carried by `.failed` events, in yield order.
  var failedIDs: [UUID] {
    compactMap {
      if case let .failed(id) = $0 { return id }
      return nil
    }
  }
  ...
  var resentIDs: [UUID] {
    compactMap {
      if case let .resent(id) = $0 { return id }
      return nil
    }
  }
  ...
  var resolvedIDs: [UUID] {
    compactMap {
      if case let .statusResolved(id, _, _) = $0 { return id }
      return nil
    }
  }
  ...
  var retryUpdates: [(messageID: UUID, attempt: Int, maxAttempts: Int)] {
    compactMap {
      if case let .retrying(id, attempt, maxAttempts) = $0 {
