...
struct PersistenceStoreBatchSyncTests {
    ...
    @Test("batchSaveChannels inserts new and updates existing in one pass")
    func batchSaveChannelsInsertsAndUpdates() async throws {
        let radioID = UUID()
    ...
    @Test("batchSaveChannels deletes stale rows at unconfigured indices")
    func batchSaveChannelsDeletesUnconfigured() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
    ...
    @Test("batchSaveChannels prunes orphans beyond device capacity")
    func batchSaveChannelsPrunesOrphans() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
    ...
    @Test("batchSaveChannels leaves circuit-breaker-skipped slots untouched")
    func batchSaveChannelsLeavesSkippedSlotsUntouched() async throws {
    ...
    @Test("batchSaveChannels handles non-contiguous configured indices")
    func batchSaveChannelsHandlesGaps() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
    ...
    @Test("batchSaveChannels on empty store with no work returns empty")
    func batchSaveChannelsEmpty() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
    ...
    @Test("batchSaveChannels scopes to radioID")
    func batchSaveChannelsScopesToRadio() async throws {
        let radioA = UUID()
    ...
    @Test("batchSaveContacts inserts all and returns count")
    func batchSaveContactsInserts() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
    ...
    @Test("batchSaveContacts updates existing rows and preserves favorite bit")
    func batchSaveContactsUpdatesAndPreservesFavorite() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
    ...
    @Test("batchSaveContacts with empty frames returns zero")
    func batchSaveContactsEmpty() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
