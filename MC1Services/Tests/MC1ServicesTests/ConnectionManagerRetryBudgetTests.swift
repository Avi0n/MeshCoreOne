...
struct ConnectionManagerRetryBudgetTests {
  ...
  struct RetryBudgetCase: CustomTestStringConvertible {
    let hasSystemPairingRegistry: Bool
  ...
  func `connect uses the unverified budget only for a macOS user-initiated tap, the full budget otherwise`(_ testCase: RetryBudgetCase) async throws {
  ...
  @Test
  func `connect rejects an unconnectable registered device before any transport attempt`() async throws {
    let container = try PersistenceStore.createContainer(inMemory: true)
    let suiteName = "test.\(UUID().uuidString)"
    let defaults = try #require(UserDefaults(suiteName: suiteName))
    defer { UserDefaults().removePersistentDomain(forName: suiteName) }
