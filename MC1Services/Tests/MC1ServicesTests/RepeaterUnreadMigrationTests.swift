...
struct RepeaterUnreadMigrationTests {
    ...
    @Test("Repeater contact unread is zeroed; chat contact untouched")
    func clearsRepeaterContactsOnly() async throws {
    ...
    @Test("Repeater-role session unread is zeroed; room session untouched")
    func clearsRepeaterSessionsOnly() async throws {
        let store = try await createTestStore()
        await store.resetRepeaterUnreadMigrationFlag()
        let radioID = UUID()
