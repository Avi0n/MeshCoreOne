...
struct NodeConfigServiceTests {
  ...
  @Test
  func `buildRadioSettings converts MHz frequency to kHz`() {
  ...
  @Test
  func `buildRadioSettings converts kHz bandwidth to Hz`() {
    let radio = NodeConfigService.buildRadioSettings(from: Self.testSelfInfo)

    // 62.5 kHz → 62500 Hz
    #expect(radio.bandwidth == 62500)
  }

  @Test
  func `buildRadioSettings copies spreading factor, coding rate, and tx power`() {
    let radio = NodeConfigService.buildRadioSettings(from: Self.testSelfInfo)
  ...
  @Test
  func `buildRadioSettings rounds frequency to the nearest kHz`() {
    let info = SelfInfo(
      advertisementType: 1,
      txPower: 22,
      maxTxPower: 30,
  ...
  @Test
  func `buildOtherSettings maps manualAddContacts=false to 0`() {
    let other = NodeConfigService.buildOtherSettings(from: Self.testSelfInfo)
    #expect(other.manualAddContacts == 0)
  }

  @Test
  func `buildOtherSettings maps manualAddContacts=true to 1`() {
    let info = SelfInfo(
  ...
  @Test
  func `buildOtherSettings exports only 2 companion-app fields`() {
    let other = NodeConfigService.buildOtherSettings(from: Self.testSelfInfo)
  ...
  @Test
  func `buildContactConfig populates all fields from MeshContact`() {
  ...
  @Test
  func `buildContactConfig includes hex outPath for routed contacts`() {
    let config = NodeConfigService.buildContactConfig(from: Self.testContact)
    #expect(config.outPath == "aabbcc")
  }

  @Test
  func `buildContactConfig uses nil outPath for flood routing`() {
    let config = NodeConfigService.buildContactConfig(from: Self.floodContact)
    #expect(config.outPath == nil)
  }

  @Test
  func `buildContactConfig uses empty string outPath for direct (zero-length) path`() {
    let config = NodeConfigService.buildContactConfig(from: Self.zeroPathContact)
    #expect(config.outPath == "")
  }

  @Test
  func `buildContactConfig truncates outPath to outPathLength bytes`() {
  ...
  @Test
  func `stepCount sums one step per resolved write plus two for radio`() throws {
  ...
  @Test
  func `Two same-secret channels still count as two write steps`() throws {
  ...
  @Test
  func `stepCount is zero for an empty plan`() throws {
    let plan = try planConfigImport(
  ...
  @Test
  func `Partial OtherSettings fills missing fields from current device values`() {
  ...
  @Test
  func `Full OtherSettings uses all imported values`() {
    let imported = MeshCoreNodeConfig.OtherSettings(
      manualAddContacts: 0,
  ...
  @Test
  func `buildRadioSettings round-trips through config format`() {
    let radio = NodeConfigService.buildRadioSettings(from: Self.testSelfInfo)
  ...
  @Test
  func `buildContactConfig and import produce consistent outPath`() {
  ...
  @Test
  func `buildContactConfig and import produce consistent flood path`() {
  ...
  @Test
  func `Direct contact round-trips through export and import without becoming flood`() throws {
    ...
    let outPathLength: UInt8 = if let pathHex = reimported.outPath, !pathHex.isEmpty {
  ...
  @Test
  func `buildContactConfig exports pathHashMode for mode 0 contact`() {
    let config = NodeConfigService.buildContactConfig(from: Self.testContact)
  ...
  @Test
  func `buildContactConfig exports pathHashMode for mode 1 (2-byte) contact`() {
  ...
  @Test
  func `buildContactConfig exports nil pathHashMode for flood contacts`() {
    let config = NodeConfigService.buildContactConfig(from: Self.floodContact)
  ...
  @Test
  func `ContactConfig import with pathHashMode encodes outPathLength correctly`() throws {
    let json = """
    {
  ...
  @Test
  func `ContactConfig import without pathHashMode defaults to mode 0`() throws {
    let json = """
    {
        "type": 1, "name": "Test", "public_key": "\(String(repeating: "ab", count: 32))",
        "flags": 0, "latitude": "0", "longitude": "0",
  ...
  @Test
  func `Direct contact with pathHashMode imports as outPathLength 0, not mode-encoded`() throws {
    let json = """
    {
    ...
    let outPathLength: UInt8 = if let pathHex = config.outPath, !pathHex.isEmpty {
  ...
  @Test
  func `NodeConfigServiceError has descriptive messages`() {
  ...
  @Test
  func `ImportProgress stores step info`() {
  ...
  @Test
  func `resolveEffectiveRadioID returns callback result when private key was imported`() async throws {
    let original = UUID()
    let reconciled = UUID()
  ...
  @Test
  func `resolveEffectiveRadioID skips callback when no private key was imported`() async throws {
    actor CallTracker {
      var calls = 0
      func bump() {
        calls += 1
  ...
  @Test
  func `resolveEffectiveRadioID returns original when callback returns nil`() async throws {
    let original = UUID()
    let result = try await resolveEffectiveRadioID(
      original: original,
      didImportPrivateKey: true,
  ...
  @Test
  func `resolveEffectiveRadioID handles nil callback gracefully`() async throws {
    let original = UUID()
    let result = try await resolveEffectiveRadioID(
      original: original,
      didImportPrivateKey: true,
  ...
  private static func radioExecutePlan() throws -> ConfigImportPlan {
    let config = MeshCoreNodeConfig(
      name: "Node",
      privateKey: String(repeating: "ab", count: 64),
      radioSettings: .init(frequency: 910_525, bandwidth: 62500,
  ...
  @Test
  func `Execute applies identity before channels/contacts and reports one step per write`() async throws {
  ...
  @Test
  func `A first-write failure reports no progress, so the import reads as clean, not partial`() async throws {
    let plan = try Self.executePlan()
    let spy = ExecuteSpy()
  ...
  @Test
  func `A mid-sequence failure reports progress only for the writes that already succeeded`() async throws {
    let plan = try Self.executePlan()
    let spy = ExecuteSpy()
    await #expect(throws: NodeConfigServiceError.self) {
      try await executeConfigImport(
  ...
  @Test
  func `A local-save failure after the device add still reports the contact as applied`() async throws {
  ...
  @Test
  func `Execute writes radio after contacts and tx power after radio params`() async throws {
  ...
  @Test
  func `A tx-power failure after radio params rethrows and reports radio progress but not tx power`() async throws {
    let plan = try Self.radioExecutePlan()
    let spy = ExecuteSpy()
    await #expect(throws: NodeConfigServiceError.self) {
      try await executeConfigImport(
  ...
  @Test
  func `Execute over a full plan emits exactly stepCount progress steps across all branches`() async throws {
    let plan = try Self.fullSectionsPlan()
    let spy = ExecuteSpy()
    try await executeConfigImport(
  ...
  @Test
  func `Pref decision gates: each setter fires only when its field differs from the device`() {
    let info = Self.matchingSelfInfo()
    let radio = MeshCoreNodeConfig.RadioSettings(
      frequency: 910_525, bandwidth: 62500, spreadingFactor: 7, codingRate: 5, txPower: 20
    )
    ...
    var sfChanged = radio
    sfChanged = .init(frequency: 910_525, bandwidth: 62500, spreadingFactor: 8, codingRate: 5, txPower: 20)
    #expect(radioParamsNeedWrite(sfChanged, current: info) == true)

    #expect(txPowerNeedsWrite(radio, current: info) == false)
    #expect(txPowerNeedsWrite(.init(frequency: 910_525, bandwidth: 62500, spreadingFactor: 7, codingRate: 5, txPower: 19), current: info) == true)
  }

  @Test
  func `An unchanged config skips every pref/radio device write but still reports progress`() async throws {
    let plan = try Self.fullSectionsPlan()
    let spy = ExecuteSpy()
    let info = Self.matchingSelfInfo()
    try await executeConfigImport(
  ...
  @Test
  func `A TX-power-only change writes only setTxPower among the radio writes`() async throws {
...
private final class ExecuteSpy: @unchecked Sendable {
  ...
  func record(_ label: String) {
    lock.lock(); _calls.append(label); lock.unlock()
  }

  func recordProgress(_ step: ImportStep) {
    lock.lock(); _progress.append(step); lock.unlock()
  }
...
struct NodeConfigImportOtherParamsLiveTests {
  @Test
  @MainActor
  func `importOtherParams forwards an unmodeled advert_loc_policy byte to the device verbatim`() async throws {
  ...
  @Test
  @MainActor
  func `importOtherParams whose merge matches the device sends no setOtherParams commit`() async throws {
    let transport = MockTransport()
    let session = MeshCoreSession(transport: transport)
