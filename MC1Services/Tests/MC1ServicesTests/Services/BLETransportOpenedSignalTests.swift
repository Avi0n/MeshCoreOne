...
struct BLETransportOpenedSignalTests {
    ...
    @Test("wait returns immediately when signal already armed")
    func waitReturnsImmediately() async throws {
        let triggers = BLETransportOpenedSignal()
        await triggers.fire()
        try await triggers.wait()
    }

    @Test("wait suspends until fire")
    func waitSuspendsUntilFire() async throws {
        let triggers = BLETransportOpenedSignal()
        let waiting = Task {
            try await triggers.wait()
        }
    ...
    @Test("clear drops armed-pending signal")
    func clearDropsArmed() async throws {
        let triggers = BLETransportOpenedSignal()
        await triggers.fire()
    ...
    @Test("wait throws CancellationError when calling task is cancelled")
    func waitThrowsOnCancellation() async {
        let triggers = BLETransportOpenedSignal()
        let task = Task {
            try await triggers.wait()
        }
    ...
    @Test("fire after cancellation does not double-resume")
    func fireAfterCancellationDoesNotDoubleResume() async throws {
        let triggers = BLETransportOpenedSignal()
        let task = Task {
            try await triggers.wait()
        }
