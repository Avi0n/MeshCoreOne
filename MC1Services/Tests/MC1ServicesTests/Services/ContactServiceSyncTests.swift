...
struct ContactServiceSyncTests {
    ...
    @Test("Full sync persists all device contacts and prunes locals not on device")
    func fullSyncPersistsAndPrunes() async throws {
        let radioID = UUID()
    ...
    @Test("Incremental sync upserts without pruning unseen locals")
    func incrementalSyncDoesNotPrune() async throws {
        let radioID = UUID()
        let store = try await PersistenceStore.createTestDataStore(radioID: radioID, maxChannels: 8)
