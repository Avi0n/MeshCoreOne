...
struct SettingsServiceDefaultFloodScopeTests {
    ...
    @Test("setDefaultFloodScopeVerified truncates overlong names before send and verify")
    @MainActor
    func setDefaultFloodScopeVerified_truncatesOverlongName() async throws {
    ...
    @Test("setDefaultFloodScopeVerified forwards names at or below the cap unchanged")
    @MainActor
    func setDefaultFloodScopeVerified_passesShortNameThrough() async throws {
        let name = "Germany"
    ...
    @Test("setDefaultFloodScopeVerified clears the scope when name is nil")
    @MainActor
    func setDefaultFloodScopeVerified_clear() async throws {
        let (service, session, transport) = try await makeService()
        defer { Task { await session.stop() } }
    ...
    private func makeDefaultFloodScopePacket(name: String) -> Data {
        ...
        while nameField.count < 31 { nameField.append(0) }
