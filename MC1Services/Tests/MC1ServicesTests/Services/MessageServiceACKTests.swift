...
struct MessageServiceACKTests {
  ...
  @Test
  func `isAckExpiryCheckingActive toggles correctly`() async throws {
  ...
  @Test
  func `stopAckExpiryChecking cancels the background task`() async throws {
    let (service, _) = try await MessageService.createForTesting()
  ...
  @Test
  func `checkExpiredAcks marks expired ACK as failed`() async throws {
  ...
  @Test
  func `ACK timeout keeps message .sent through the grace window so a late ACK can still reconcile`() async throws {
  ...
  @Test
  func `checkExpiredAcks preserves non-expired ACK`() async throws {
    let (service, _) = try await MessageService.createForTesting()
  ...
  @Test
  func `checkExpiredAcks skips already-delivered ACK`() async throws {
    let (service, _) = try await MessageService.createForTesting()
  ...
  @Test
  func `checkExpiredAcks does not broadcast failure when DB stays delivered`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `checkExpiredAcks fails a DM after max(ackGiveUpWindow, per-attempt timeout); the window is the floor`() async throws {
  ...
  @Test
  func `checkExpiredAcks honors a per-attempt timeout longer than the give-up window (slow preset)`() async throws {
    let window: TimeInterval = 20
  ...
  @Test
  func `stopAckExpiryChecking leaves in-flight DMs .sent instead of failing them`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
    try await dataStore.saveMessage(
  ...
  @Test
  func `failAllPendingMessages fails all non-delivered and broadcasts .failed`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID1 = UUID()
    let messageID2 = UUID()
  ...
  @Test
  func `stopAndFailAllPending stops checking and fails all pending`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `handleAcknowledgement uses firmware tripTime when provided`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `handleAcknowledgement leaves roundTripTime nil when firmware does not supply tripTime`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `handleAcknowledgement matches any CRC accumulated across retry attempts`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `handleAcknowledgement updates contact lastMessageDate on late ACK`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
  ...
  @Test
  func `trackPendingAck on retry resets sentAt so checkExpiredAcks preserves a retrying message`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
    let contactID = UUID()
  ...
  @Test
  func `finalizeSend preserves roundTripTime after listener-won delivery`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
    let contactID = UUID()
    let radioID = testDeviceID
  ...
  @Test
  func `finalizeSend broadcasts .statusResolved when in-loop waitForEvent wins the ACK race`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
    let contactID = UUID()
    let radioID = testDeviceID
  ...
  @Test
  func `handleAcknowledgement is a no-op when no entry matches the ackCode`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `failMessageAndRethrow removes pendingAcks entry and rethrows`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `pendingAckCount reflects count correctly`() async throws {
    let (service, _) = try await MessageService.createForTesting()
  ...
  @Test
  func `ACK within grace window reconciles .sent → .delivered via the in-memory pending entry`() async throws {
  ...
  @Test
  func `finalizeSend delivered branch writes .delivered, updates contact, yields .statusResolved, removes entry`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
    let contactID = UUID()
    let radioID = testDeviceID
  ...
  @Test
  func `finalizeSend nil branch skips the DB write when the listener already delivered`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `finalizeSend nil branch does not re-stamp the pending entry's sentAt`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `finalizeSend nil branch over an already-.failed row leaves it .failed`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
  ...
  @Test
  func `finalizeSend nil branch yields .statusResolved(.sent) from a non-terminal row and gates off on a terminal one`() async throws {
  ...
  @Test
  func `late ACK landing in the checker's await-gap cannot flip a just-failed row to .delivered`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()
    let contactID = UUID()
  ...
  @Test
  func `late ACK after the row is already .failed with no live entry stays .failed`() async throws {
    let (service, dataStore) = try await MessageService.createForTesting()
    let messageID = UUID()

    try await dataStore.saveMessage(
      MessageDTO.testDirectMessage(id: messageID, radioID: testDeviceID, status: .failed, ackCode: 0xABCD_EF01)
    )
