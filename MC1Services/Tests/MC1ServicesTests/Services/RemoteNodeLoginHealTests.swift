...
struct RemoteNodeLoginHealTests {
  ...
  @Test
  func `notFound on login pushes the local contact to the radio and retries`() async throws {
    let radioID = UUID()
  ...
  @Test
  func `healing preserves a contact type byte not modeled by ContactType`() async throws {
    let radioID = UUID()
    let dataStore = try await PersistenceStore.createTestDataStore(radioID: radioID)
  ...
  @Test
  func `a non-notFound login error skips healing and is not retried`() async throws {
    let radioID = UUID()
    let dataStore = try await PersistenceStore.createTestDataStore(radioID: radioID)
    let contact = ContactDTO.testContact(
      radioID: radioID,
    ...
    guard case let .deviceError(code) = thrown, code == ProtocolError.tableFull.rawValue else {
  ...
  @Test
  func `a second notFound after re-adding the contact surfaces the device error unhealed`() async throws {
    let radioID = UUID()
    let dataStore = try await PersistenceStore.createTestDataStore(radioID: radioID)
    let contact = ContactDTO.testContact(
      radioID: radioID,
    ...
    guard case let .deviceError(code) = thrown, code == ProtocolError.notFound.rawValue else {
  ...
  @Test
  func `a full radio contact table surfaces radioContactsFull`() async throws {
    let radioID = UUID()
    let dataStore = try await PersistenceStore.createTestDataStore(radioID: radioID)
    let contact = ContactDTO.testContact(
      radioID: radioID,
  ...
  @Test
  func `a contact absent from the local store cannot be healed`() async throws {
    let radioID = UUID()
    let dataStore = try await PersistenceStore.createTestDataStore(radioID: radioID)
  ...
  @Test
  func `login() surfaces radioContactsFull through the continuation instead of a generic session error`() async throws {
    let radioID = UUID()
    let dataStore = try await PersistenceStore.createTestDataStore(radioID: radioID)
