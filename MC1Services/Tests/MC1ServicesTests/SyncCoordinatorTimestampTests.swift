...
struct SyncCoordinatorTimestampTests {
  ...
  @Test
  func `Timestamp within valid range is not corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp 1 minute in future is not corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp exactly 5 minutes in future is not corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp 6 minutes in future is corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp 1 week ago is not corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp 3 months ago is not corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp exactly 6 months in past is not corrected`() {
  ...
  @Test
  func `Timestamp 7 months ago is corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp of zero (Unix epoch) is corrected`() {
    let now = Date()
  ...
  @Test
  func `Timestamp from year 2020 is corrected`() {
    let now = Date()
    let oldDate = Date(timeIntervalSince1970: 1_577_836_800) // Jan 1, 2020
  ...
  @Test
  func `Timestamp from year 2030 is corrected`() {
    let now = Date()
    let futureDate = Date(timeIntervalSince1970: 1_893_456_000) // Jan 1, 2030
    let timestamp = UInt32(futureDate.timeIntervalSince1970)
  ...
  @Test
  func `Original timestamp is preserved when correction is applied`() {
  ...
  @Test
  func `Corrected timestamp differs from original for invalid input`() {
    let now = Date()
  ...
  @Test
  func `Receive time near Unix epoch does not crash`() {
  ...
  @Test
  func `Receive time at Unix epoch handles timestamp validation`() {
...
struct SyncCoordinatorSortDateTests {
  ...
  @Test
  func `Live message sorts by receive time`() {
    let now = Date()
  ...
  @Test
  func `Backlog message sorts by the drain anchor, not its receive time`() {
  ...
  @Test
  func `Every message in one drain shares the anchor as its sort date`() {
    let anchor = Date()
  ...
  @Test
  func `Distinct drains sort as distinct blocks in delivery order`() {
...
struct SameSenderReorderingTests {
  ...
  @Test
  func `Empty array returns empty`() {
    let result = MessageDTO.reorderSameSenderClusters([])
    #expect(result.isEmpty)
  }

  @Test
  func `Single message returns unchanged`() {
  ...
  @Test
  func `DM messages within 5 seconds are reordered by sender timestamp`() {
    let base = Date()
  ...
  @Test
  func `Outgoing DM messages within 5 seconds are reordered by sender timestamp`() {
    let base = Date()
  ...
  @Test
  func `DM messages beyond 5 seconds are not reordered`() {
    let base = Date()
  ...
  @Test
  func `Channel messages from different senders are not clustered`() {
    let base = Date()
  ...
  @Test
  func `Channel messages from same sender within window are reordered`() {
    let base = Date()
  ...
  @Test
  func `Mixed directions break clusters`() {
    let base = Date()
  ...
  @Test
  func `Three messages in cluster are fully sorted`() {
    let base = Date()
  ...
  @Test
  func `Exactly 5 second gap is included in cluster`() {
    let base = Date()
  ...
  @Test
  func `Multiple consecutive clusters are each reordered independently`() {
    let base = Date()
  ...
  @Test
  func `Channel messages with nil sender names are not clustered`() {
    let base = Date()
  ...
  @Test
  func `Same-sender messages with identical timestamps use createdAt as tiebreaker`() {
    let base = Date()
    let msg1 = makeDMMessage(timestamp: 100, createdAt: base)
  ...
  @Test
  func `Messages already in correct order are unchanged`() {
    let base = Date()
    let msg1 = makeDMMessage(timestamp: 100, createdAt: base)
  ...
  @Test
  func `Far-apart sortDates are not clustered even when createdAt order is inverted`() {
    let base = Date()
  ...
  @Test
  func `Clustering window follows sortDate, not createdAt`() {
    let base = Date()
