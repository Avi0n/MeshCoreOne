...
struct PendingSendPersistenceTests {
  @Test
  func `PendingSendDTO round-trips through Model and back`() async throws {
  ...
  @Test
  func `Pending sends are scoped by radioID`() async throws {
    let store = try makeStore()
    let radioA = UUID()
    let radioB = UUID()
  ...
  @Test
  func `fetchPendingSends returns rows in sequence order`() async throws {
    let store = try makeStore()
    let radioID = UUID()
  ...
  @Test
  func `deletePendingSend removes the row`() async throws {
    let store = try makeStore()
    let radioID = UUID()
  ...
  @Test
  func `insertPendingSendAssigningSequence returns 1 on an empty table`() async throws {
    let store = try makeStore()
    let radioID = UUID()
  ...
  @Test
  func `insertPendingSendAssigningSequence assigns per-radio monotonic sequences`() async throws {
    let store = try makeStore()
    let radioA = UUID()
    let radioB = UUID()
  ...
  @Test
  func `Concurrent inserts on the same radio produce distinct sequences`() async throws {
    let store = try makeStore()
    let radioID = UUID()
  ...
  @Test
  func `attemptCount round-trips through insertPendingSendAssigningSequence and fetchPendingSends`() async throws {
    let store = try makeStore()
    let radioID = UUID()
  ...
  @Test
  func `deletePendingSendsForMessage removes every row matching the messageID`() async throws {
    let store = try makeStore()
    let radioA = UUID()
    let radioB = UUID()
  ...
  @Test
  func `deletePendingSendsForMessage is a no-op when no rows match`() async throws {
    let store = try makeStore()
    let radioID = UUID()
    let messageID = UUID()
  ...
  @Test
  func `fetchPendingSends skips rows whose kindRawValue is unknown`() async throws {
  ...
  @Test
  func `replacePendingSendForRetry flips status and inserts a new PendingSend row`() async throws {
    let store = try makeStore()
    let radioID = UUID()
    let messageID = UUID()
    let contactID = UUID()
  ...
  @Test
  func `replacePendingSendForRetry preserves .delivered status`() async throws {
    let store = try makeStore()
    let radioID = UUID()
    let messageID = UUID()
    let contactID = UUID()
  ...
  @Test
  func `replacePendingSendForRetry removes every existing PendingSend row for the messageID`() async throws {
    let store = try makeStore()
    let radioID = UUID()
    let messageID = UUID()
    let contactID = UUID()
  ...
  /// transaction.
  @Test
  func `replacePendingSendForRetry assigns the next per-radio sequence`() async throws {
    let store = try makeStore()
    let radioID = UUID()
