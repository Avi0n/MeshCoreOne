...
struct IdentityReconciliationIntegrationTests {
    ...
    @Test("Remove + erase/flash + re-pair + import config re-links orphaned children")
    func endToEndReconciliation() async throws {
    ...
    @Test("No reconciliation when radio's publicKey didn't change after re-pair")
    func reconciliationSkippedWhenPublicKeyStable() async throws {
        let store = try await createStore()
    ...
    @Test("Reconciliation does not migrate post-pair child rows to the reconciled radioID")
    func reconciliationDoesNotMigratePostPairChildren() async throws {
        let store = try await createStore()
