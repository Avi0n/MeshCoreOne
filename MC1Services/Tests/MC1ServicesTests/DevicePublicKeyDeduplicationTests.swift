...
struct DevicePublicKeyDeduplicationTests {
  ...
  @Test
  func `fetchDevice(publicKey:) returns matching device`() async throws {
  ...
  @Test
  func `fetchDevice(publicKey:) returns nil for unknown key`() async throws {
    let store = try await createTestStore()
  ...
  @Test
  @MainActor
  func `createDevice preserves radioID from existing device`() throws {
  ...
  @Test
  @MainActor
  func `createDevice uses the provided radioID for new pairings`() throws {
    let (cm, _) = try ConnectionManager.createForTesting()
    let bleUUID = UUID()
  ...
  @Test
  @MainActor
  func `createDevice persists the Bluetooth method supplied by the BLE connect path`() throws {
    let (cm, _) = try ConnectionManager.createForTesting()
    let bleUUID = UUID()
  ...
  @Test
  @MainActor
  func `createDevice merges a Bluetooth method with an existing WiFi method`() throws {
    let (cm, _) = try ConnectionManager.createForTesting()
    let bleUUID = UUID()
    let existingRadioID = UUID()
    let existingDevice = DeviceDTO.testDevice(
  ...
  @Test
  @MainActor
  func `createDevice replaces a stale Bluetooth peripheral UUID instead of accumulating`() throws {
    let (cm, _) = try ConnectionManager.createForTesting()
    let existingRadioID = UUID()
    let staleUUID = UUID()
    let existingDevice = DeviceDTO.testDevice(
    ...
    guard case let .bluetooth(uuid, _) = bluetoothMethods.first else {
