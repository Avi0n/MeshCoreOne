...
extension SendMessageIntent {
  ...
  static func mapToIntentError(_ error: Error) -> IntentError {
    ...
    case let serviceError as MessageServiceError:
      ...
      case .invalidRecipient, .contactNotFound, .channelNotFound: .invalidRecipient
      case .messageTooLong: .messageTooLong
      case let .sessionError(underlying): .sessionError(underlying)
  ...
  private static func recipientName(for recipient: MessageRecipient) -> String {
    switch recipient {
    case let .contact(dto): dto.displayName
    case let .channel(dto): dto.displayName
    }
  }
}
