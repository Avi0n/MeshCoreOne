...
extension TerrainProfileCanvas {
  ...
  private func niceStep(for range: Double, targetDivisions: Int) -> Double {
    ...
    let niceNormalized: Double = if normalized <= 1 {
      1
...
extension TerrainProfileCanvas {
  private func drawGrid(context: GraphicsContext, coords: ChartCoordinateSpace) {
    ...
    for (index, x) in xTicks.enumerated() {
      ...
      let labelText: String = if xStep >= 1000, kmValue.truncatingRemainder(dividingBy: 1) == 0 {
        isLast ? "\(Int(kmValue)) km" : "\(Int(kmValue))"
      } else {
        isLast
  ...
  private func drawTerrain(context: GraphicsContext, coords: ChartCoordinateSpace) {
    ...
    let allSamples: [ProfileSample] = if profileSamplesRB.isEmpty {
      profileSamples
    } else {
      profileSamples + profileSamplesRB.dropFirst()
    }
