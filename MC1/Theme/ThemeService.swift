...
final class ThemeService {
    ...
    init(store: StoreService, defaults: UserDefaults = .standard) {
        ...
        if let id = storedID, let theme = ThemeRegistry.theme(forID: id) {
            self.current = theme                       // adopt as-is; no write-back
        } else if storedID == nil {
            self.current = .default                    // missing key: default in memory, no write-back
        } else {
            self.current = .default                    // unknown ID: fall back and overwrite the stale value
        ...
        if let raw = storedScheme, let preference = AppColorSchemePreference(rawValue: raw) {
            ...
            self.colorSchemePreference = .system       // missing key: no write-back
        } else {
            self.colorSchemePreference = .system       // unknown raw value: fall back and overwrite
