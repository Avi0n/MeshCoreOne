...
struct ThemeContrastTests {
    ...
    @Test("outgoing text clears WCAG AA 4.5:1 against the accent in every appearance")
    func outgoingTextClearsAAFloor() {
    ...
    @Test("incoming hashtag links clear WCAG AA 4.5:1 against the incoming bubble in every appearance")
    func incomingHashtagClearsAAFloor() {
        for theme in Self.paidThemes {
            for trait in Self.traits(for: theme) {
    ...
    @Test("identity colors clear AA against their surfaces for every theme and appearance")
    func identityColorsClearAA() {
    ...
    @Test("gamut-derived category colors clear AA against the list canvas")
    func categoryColorsClearAA() {
    ...
    @Test("channel, repeater, and room avatars resolve to distinct on-anchor hues for every gamut theme")
    ...
    @Test("avatar glyph clears AA against the identity fill for every theme and appearance")
    func glyphClearsAA() {
        for theme in ThemeRegistry.allThemes {
            for trait in Self.traits(for: theme) {
                let backgrounds = Self.surfaceLuminances(theme, trait.collection)
