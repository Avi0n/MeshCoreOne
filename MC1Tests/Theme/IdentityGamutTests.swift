...
struct IdentityGamutTests {
    ...
    @Test("every identity color clears AA against its surfaces in both appearances and contrasts")
    ...
    @Test("avatar glyph clears AA against the identity fill in both appearances")
    func glyphClearsAA() {
        for trait in [Self.light, Self.dark] {
            let backgrounds = surfaceLuminances(trait)
            for name in Self.names {
    ...
    @Test("hue is stable across appearance and contrast for a given name")
    func hueStableAcrossAppearance() {
