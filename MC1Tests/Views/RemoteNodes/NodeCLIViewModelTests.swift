...
struct NodeCLIViewModelTests {
    ...
    @Test("Configure shows a banner naming the node")
    func configureShowsBanner() {
    ...
    @Test("configure is idempotent: re-configuring does not duplicate the banner")
    ...
    @Test("help prints help and does not send")
    func helpPrintsHelpNoSend() async throws {
        let spy = SendSpy()
    ...
    @Test("bare clear empties output and does not send")
    func bareClearEmptiesOutputNoSend() async throws {
        let spy = SendSpy()
        let viewModel = makeViewModel(spy: spy)
        viewModel.executeCommand("help")
    ...
    @Test("clear stats sends the raw command")
    func clearStatsSends() async throws {
        let spy = SendSpy()
        let viewModel = makeViewModel(spy: spy)
    ...
    @Test("reboot sends with a 2s timeout and renders timeout as success")
    func rebootTreatsTimeoutAsSuccess() async throws {
        let spy = SendSpy()
    ...
    @Test("reboot now is treated as a reboot")
    func rebootNowTreatedAsReboot() async throws {
        let spy = SendSpy()
        let viewModel = makeViewModel(spy: spy)
    ...
    @Test("a reboot typo is sent as a normal command, not treated as a reboot")
    func rebootTypoNotTreatedAsReboot() async throws {
        let spy = SendSpy()
    ...
    @Test("a cancelled reboot does not surface a raw CancellationError")
    func rebootSwallowsCancellationError() async throws {
        let spy = SendSpy()
    ...
    @Test("arbitrary command sends with default timeout and appends response")
    func arbitraryCommandAppendsResponse() async throws {
        let spy = SendSpy()
        spy.result = "af: 1"
        let viewModel = makeViewModel(spy: spy)
    ...
    @Test("a thrown error appends an .error line")
    func thrownErrorAppendsErrorLine() async throws {
        let spy = SendSpy()
    ...
    @Test("Two view models with two closures each only invoke their own")
    ...
    @Test("Output is capped at 1000 lines")
    func outputCappedAt1000() async throws {
        let viewModel = NodeCLIViewModel()
    ...
    func historyCappedAt100() {
        let viewModel = makeViewModel(spy: SendSpy())
        for i in 0..<150 {
            viewModel.executeCommand("command\(i)")
        }
        for _ in 0..<100 { viewModel.historyUp() }
        #expect(viewModel.currentInput == "command50")
    }

    @Test("getResponseBlock walks the full multi-line response block, not just one line")
    func getResponseBlockWalksFullBlock() async throws {
        let viewModel = makeViewModel(spy: SendSpy())
        viewModel.executeCommand("help")
        try await waitUntil("help output appears") {
            viewModel.outputLines.contains { $0.text.contains("Available commands") }
