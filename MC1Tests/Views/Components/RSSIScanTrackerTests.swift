...
struct RSSIScanTrackerTests {
  ...
  @Test
  func `consume ingests a usable discovery and exposes its name and tier`() async {
  ...
  @Test
  func `a later packet without a name keeps the previously advertised name and smooths RSSI`() async {
    let tracker = RSSIScanTracker()
    let id = UUID()
  ...
  @Test
  func `unknown ids report no tier and are not advertising`() {
    let tracker = RSSIScanTracker()
  ...
  func `an unusable reading is skipped and creates no entry`(rssi: Int) async {
    let tracker = RSSIScanTracker()
    let id = UUID()
  ...
  @Test
  func `expireStale drops a peripheral last seen before the stale window`() async {
    let tracker = RSSIScanTracker()
    let id = UUID()
    await tracker.consume(finishedStream([
      DiscoveredDevice(id: id, name: "Radio", rssi: -50)
  ...
  @Test
  func `expireStale keeps a freshly seen peripheral`() async {
    let tracker = RSSIScanTracker()
    let id = UUID()
    await tracker.consume(finishedStream([
      DiscoveredDevice(id: id, name: "Radio", rssi: -50)
