...
struct RSSITuningTests {
  ...
  func `isUsable accepts negative dBm readings and rejects the unavailable sentinels`(rssi: Int, expected: Bool) {
  ...
  @Test
  func `smooth returns the new sample unchanged when there is no prior reading`() {
    #expect(RSSITuning.smooth(newRSSI: -50, previousRSSI: nil) == -50)
  }

  @Test
  func `smooth weights the newest sample at 0.2 against the previous smoothed value`() {
  ...
  func `first reading maps directly at the tier thresholds`(rssi: Int, expected: RSSITuning.SignalTier) {
  ...
  @Test
  func `a reading inside the hysteresis band holds the current tier`() {
  ...
  @Test
  func `a reading clearing the band by the hysteresis margin flips the tier`() {
  ...
  @Test
  func `from medium, a reading moves up, drops, or holds at the band edges`() {
  ...
  @Test
  func `a single strong reading jumps weak straight to strong`() {
  ...
  func `fillLevel maps each tier to its glyph fill`(tier: RSSITuning.SignalTier, expected: Double) {
    #expect(RSSITuning.fillLevel(forTier: tier) == expected)
  }

  @Test
  func `color maps each tier to its glyph color`() {
