...
struct ChatViewModelCoordinatorSharingTests {
    ...
    @Test("Two ChatViewModels on the same conversation share one ChatCoordinator")
    func splitView_sharesCoordinator() throws {
    ...
    @Test("Distinct conversations get distinct coordinators")
    func distinctConversations_distinctCoordinators() throws {
        let registry = try makeRegistry()
        let radioID = UUID()
        let contactA = UUID()
        let contactB = UUID()
