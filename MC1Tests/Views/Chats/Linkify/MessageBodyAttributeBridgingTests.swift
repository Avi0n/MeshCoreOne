...
struct MessageBodyAttributeBridgingTests {
  ...
  private func assertLinkParity(_ swiftUI: AttributedString, _ uikit: NSAttributedString, _ label: Comment) {
    let swiftLinks = Set(swiftUI.runs.compactMap(\.link))
  ...
  @Test
  func `Incoming base color resolves to UIColor.label, not grey`() {
  ...
  @Test
  func `Self-mention background highlight bridges in light and dark/high-contrast`() {
  ...
  @Test
  func `Hashtag bold run is derived from the body text style, not a fixed size`() {
  ...
  @Test
  func `Hashtag run is the only bold run and carries a link`() {
    let swiftUI = swiftUIString("ping #news end")
    let uikit = bridged(swiftUI, traitCollection: Self.light)
  ...
  @Test
  func `configureForBubble empties linkTextAttributes`() {
  ...
  @Test
  func `linkURL(at:) resolves linked glyphs and rejects non-link and trailing taps`() throws {
  ...
  @Test
  func `linkURL(at:) does not over-hit a trailing link from a tap past it`() throws {
    let url = try #require(URL(string: "https://example.com"))
