...
struct MessageLinkAccessibilityTests {
  ...
  @Test
  func `Each body-text link kind yields a distinctly-named action in document order`() {
  ...
  @Test
  func `A shared contact link is named from its parsed contact name`() {
  ...
  @Test
  func `A shared channel link is named from its parsed channel name`() {
  ...
  @Test
  func `The preview URL is surfaced before body-text links`() throws {
    let preview = try #require(URL(string: "https://preview.example"))
    let actions = MessageLinkAccessibility.actions(
      previewURL: preview,
  ...
  @Test
  func `Duplicate URLs collapse to a single action`() {
  ...
  @Test
  func `The action count is capped`() {
  ...
  @Test
  func `No links yields no actions`() {
