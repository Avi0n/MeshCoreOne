...
struct ResolveMentionTapTests {
    ...
    @Test("Single match returns navigate outcome with the matched contact")
    func singleMatchNavigates() {
        let radio = UUID()
        ...
        if case .navigate(let contact) = outcome {
    ...
    @Test("Multiple matches returns picker outcome with all matches and isSelfMention false")
    func multipleMatchesPicker() {
        let radio = UUID()
        ...
        if case .picker(let context) = outcome {
    ...
    @Test("Zero matches with non-self name returns picker outcome with empty matches and isSelfMention false")
    func zeroMatchesNonSelf() {
        let radio = UUID()
        let outcome = MentionTapEvaluator.evaluate(
            rawName: "Charlie",
    ...
    @Test("Zero matches with self-name returns picker outcome with isSelfMention true (case-insensitive)")
    func zeroMatchesSelf() {
        let radio = UUID()
        let outcome = MentionTapEvaluator.evaluate(
            rawName: "mydevice",
            contacts: [makeContact(name: "Alice", radioID: radio)],
    ...
    @Test("Blocked single match still navigates (excludeBlocked is false)")
    func blockedSingleMatchStillNavigates() {
        let radio = UUID()
    ...
    @Test("Offline localNodeName fallback Me does NOT mark inbound @[Me] as self")
    func offlineLocalNodeFallbackNotSelf() {
        let radio = UUID()
        let outcome = MentionTapEvaluator.evaluate(
            rawName: "Me",
            contacts: [],
    ...
    @Test("Bidi-control characters are stripped from the name before matching")
    func bidiSanitization() {
        let radio = UUID()
        let alice = makeContact(name: "Alice", radioID: radio)
    ...
    @Test("Whitespace-only name short-circuits to empty picker without touching contacts")
