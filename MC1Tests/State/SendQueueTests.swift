...
struct SendQueueTests {
    ...
    @Test("Serial draining: enqueueing N envelopes triggers N sends in FIFO order")
    func serialDrainProcessesEnvelopesInOrder() async {
        actor Collector {
            var observed: [Int] = []
            func record(_ value: Int) { observed.append(value) }
            func snapshot() -> [Int] { observed }
        }
    ...
    @Test("CancellationError requeues at the front and the drain auto-respawns")
    func cancellationRequeuesAndAutoRespawns() async {
        ...
        actor Tracker {
            ...
            func recordSuccess(_ id: Int) { succeededIDs.append(id) }
            func snapshot() -> (Int, [Int]) { (sendCalls, succeededIDs) }
        }
        let tracker = Tracker()
        let drainSignal = AsyncStream.makeStream(of: Void.self)
    ...
    @Test("Non-cancellation error fires onError and the drain continues")
    func nonCancellationErrorFiresOnErrorAndContinues() async {
        struct Boom: Error {}
        actor Collector {
            var sent: [Int] = []
            var errors: [Int] = []
            func recordSend(_ id: Int) { sent.append(id) }
            func recordError(_ id: Int) { errors.append(id) }
            func sentSnapshot() -> [Int] { sent }
            func errorSnapshot() -> [Int] { errors }
        }
        let collector = Collector()
        let drainSignal = AsyncStream.makeStream(of: Void.self)
    ...
    @Test("onDrain fires exactly once per drain pass")
    func onDrainFiresOncePerPass() async {
        ...
        actor State {
            ...
            func recordSent(_ id: Int) { sent.append(id) }
    ...
    func onDrainReceivesLastError() async {
        ...
        actor ErrorBox {
            var value: Error?
            func record(_ error: Error?) { value = error }
            func snapshot() -> Error? { value }
        }
        let box = ErrorBox()
        let drainSignal = AsyncStream.makeStream(of: Void.self)
    ...
    @Test("onDrain receives nil when no envelope failed during the drain pass")
    func onDrainReceivesNilOnSuccessfulPass() async {
        actor ErrorBox {
            var value: Error?
            var didFire = false
            func record(_ error: Error?) { value = error; didFire = true }
            func snapshot() -> (Error?, Bool) { (value, didFire) }
        }
        let box = ErrorBox()
        let drainSignal = AsyncStream.makeStream(of: Void.self)
    ...
    func outerLoopProcessesEnqueueDuringOnDrain() async {
    ...
    @Test("Drain task captures actor strongly: drain completes after only external ref drops")
    func drainCompletesAfterExternalRefDrops() async {
        ...
        actor State {
            ...
            func markCompleted() { sendCompleted = true }
            func snapshot() -> Bool { sendCompleted }
        }
        let state = State()
