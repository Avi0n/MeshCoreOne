...
struct MessageEventStreamTests {
  @Test
  func `Single consumer receives an event sent via send(_:)`() async {
  ...
  @Test
  func `Multiple consumers each receive the same event`() async {
    let stream = MessageEventStream()
  ...
  @Test
  func `Events sent across consumer task restarts are not dropped when the consumer is held by a long-lived task`() async {
    let stream = MessageEventStream()
    ...
    actor Collector {
      ...
      func append(_ value: MessageEvent) {
        items.append(value)
      }

      func snapshot() -> [MessageEvent] {
        items
      }
    }
  ...
  @Test
  func `subscriberCount reflects active subscriptions`() async {
    let stream = MessageEventStream()
