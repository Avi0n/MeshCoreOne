...
struct ChatViewModelPaginationTests {
  ...
  @Test
  func `Loading fewer messages than pageSize marks no more messages available`() async throws {
  ...
  @Test
  func `loadOlderMessages prepends messages to array`() async throws {
    let dataStore = PaginationTestDataStore()
    let radioID = UUID()
    let contactID = UUID()
    let contact = createTestContact(id: contactID, radioID: radioID)
  ...
  @Test
  func `loadOlderMessages guards against concurrent fetches`() async {
    let viewModel = ChatViewModel()
  ...
  @Test
  func `Pagination state resets when loading messages for new contact`() {
    let viewModel = ChatViewModel()
    let radioID = UUID()
  ...
  @Test
  func `Initial message load sets hasMoreMessages based on count`() {
    let viewModel = ChatViewModel()
...
struct ChatViewModelChannelPaginationTests {
  @Test
  func `Channel message pagination works similar to direct messages`() async throws {
    let dataStore = PaginationTestDataStore()
    let radioID = UUID()
  ...
  @Test
  func `loadOlderMessages handles channel messages`() async {
    let viewModel = ChatViewModel()
    let radioID = UUID()
  ...
  @Test
  func `Initial channel load uses unfiltered count for hasMoreMessages`() async throws {
