...
struct SidebarNavigationLayoutTests {
    ...
    @Test("Section sidebar is narrow enough that 11-inch portrait tiles all three columns")
    func sidebarTilesInPortrait() {
        #expect(MainSidebarView.sidebarTileableMinWidth <= Self.iPad11InchPortraitWidth)
    }

    @Test("iPad mini portrait intentionally collapses rather than tiling")
    func iPadMiniPortraitCollapses() {
        #expect(MainSidebarView.sidebarTileableMinWidth > Self.iPadMiniPortraitWidth)
    }

    @Test("Sidebar width is in icon-only range, not a full sidebar")
    ...
    @Test("A sidebar-collapsing tool collapses the sidebar even when the container is wide")
    func sidebarCollapsingToolCollapsesSidebarWhenWide() {
    ...
    @Test("A wide container tiles the sidebar when no sidebar-collapsing tool is open")
    func wideContainerTilesSidebar() {
        let visibility = MainSidebarView.sidebarVisibility(
            isWide: true,
    ...
    @Test("A narrow container collapses to the section's hidden shape")
    func narrowContainerCollapsesToSectionShape() {
        #expect(
    ...
    @Test("Line of Sight and Trace Path collapse the sidebar; other tools keep it")
    func onlyWideToolsCollapseSidebar() {
