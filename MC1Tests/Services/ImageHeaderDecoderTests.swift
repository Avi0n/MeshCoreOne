...
struct ImageHeaderDecoderTests {
  ...
  @Test
  func `decodeDimensions returns pixel dims for a valid PNG`() throws {
  ...
  @Test
  func `decodeDimensions returns nil for non-image bytes`() {
    let dims = ImageHeaderDecoder.decodeDimensions(from: Data("not an image".utf8))
    #expect(dims == nil)
  }

  @Test
  func `decodeDimensions returns nil for empty data`() {
