...
struct MapSnapshotStoreTests {
  ...
  @Test
  func `A cache miss returns nil synchronously; isResolved is false`() {
  ...
  @Test
  func `Signed-zero coordinates normalize so cacheKey agrees with Hashable equality`() {
  ...
  @Test
  func `Online and offline requests for the same coordinate produce distinct cache keys`() {
  ...
  @Test
  func `A completed render caches the image and emits on the stream`() async {
  ...
  @Test
  func `A failed render marks the request resolved with no image`() async {
    let fake = FakeRenderer()
  ...
  @Test
  func `N concurrent requests for the same key enqueue exactly one render`() async {
    let fake = FakeRenderer()
    fake.awaitGate = true
    let store = MapSnapshotStore(renderer: fake)
    let req = request()
  ...
  @Test
  func `A cached request is not re-rendered`() async {
    let fake = FakeRenderer()
    let store = MapSnapshotStore(renderer: fake)
    let req = request()
  ...
  @Test
  func `The failed set is capped; the oldest entry is evicted when the limit is exceeded`() async {
    let fake = FakeRenderer()
    fake.shouldFail = true
    let store = MapSnapshotStore(renderer: fake)
  ...
  @Test
  func `clear() yields resolutions for previously-resolved requests so on-screen rows reload`() async {
    let fake = FakeRenderer()
    let store = MapSnapshotStore(renderer: fake)
    let req = request()
  ...
  @Test
  func `clearFailures() drops failed entries so the next request triggers a retry`() async {
    let fake = FakeRenderer()
    fake.shouldFail = true
    let store = MapSnapshotStore(renderer: fake)
    let req = request()
  ...
  @Test
  func `Every subscriber receives each resolution (multicast, not single-consumer split)`() async {
    let fake = FakeRenderer()
    let store = MapSnapshotStore(renderer: fake)
    let req = request()
