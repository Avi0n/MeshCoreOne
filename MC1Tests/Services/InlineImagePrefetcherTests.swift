...
struct InlineImagePrefetcherTests {
    ...
    @Test("Text with no URLs returns immediately without probes or previews")
    func noURLsInTextNoCalls() async {
    ...
    @Test("Direct image suffix invokes the dimension probe path")
    func directImageInvokesProbe() async {
        let imageCache = StubImageProber()
        let linkCache = StubLinkPreviewFetcher()
        let store = InlineImageDimensionsStore(fileURL: Self.makeTempDimensionsURL())
        let dataStore = StubDataStore()
    ...
    @Test("Multiple URLs fan out across both classifier paths")
    func multipleURLsParallel() async {
        let imageCache = StubImageProber()
        let linkCache = StubLinkPreviewFetcher()
        let store = InlineImageDimensionsStore(fileURL: Self.makeTempDimensionsURL())
        let dataStore = StubDataStore()
    ...
    @Test("Direct image probe is skipped when dimensions are already cached")
    func skipsCachedDirectImage() async {
        let imageCache = StubImageProber()
        let linkCache = StubLinkPreviewFetcher()
        let store = InlineImageDimensionsStore(fileURL: Self.makeTempDimensionsURL())
        let dataStore = StubDataStore()

        let cachedURL = URL(string: "https://example.com/cat.png")!
    ...
    @Test("Mixed direct image and link preview URLs both invoke their paths")
    func mixedDirectAndPreview() async {
        let imageCache = StubImageProber()
        let linkCache = StubLinkPreviewFetcher()
        let store = InlineImageDimensionsStore(fileURL: Self.makeTempDimensionsURL())
        let dataStore = StubDataStore()
