...
struct ChatCoordinateDetectorTests {
  @Test
  func `A valid decimal pair is detected`() {
  ...
  @Test
  func `An integer pair is not detected`() {
    #expect(ChatCoordinateDetector.firstCoordinate(in: "ratio is 3, 4 today") == nil)
  }

  @Test
  func `An out-of-range pair is rejected by the clamp, not the regex`() {
  ...
  @Test
  func `A version-like string is not detected`() {
    #expect(ChatCoordinateDetector.firstCoordinate(in: "v1.2, 3.4 release") == nil)
  }

  @Test
  func `firstCoordinate returns nil when there is no coordinate`() {
    #expect(ChatCoordinateDetector.firstCoordinate(in: "no coordinates here") == nil)
  }

  @Test
  func `firstCoordinate returns the first of several coordinates`() {
  ...
  @Test
  func `matches returns every valid coordinate in document order`() {
  ...
  @Test
  func `A coordinate ending a sentence is detected, period excluded`() {
