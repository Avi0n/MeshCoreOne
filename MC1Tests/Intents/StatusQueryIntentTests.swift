...
struct StatusQueryIntentTests {
  ...
  func `connected with battery speaks name and percent`(_ state: DeviceConnectionState) throws {
    let appState = AppState()
    let battery = BatteryInfo(level: Self.seededLevel)
    seedConnected(appState, state: state, name: "Base Camp", battery: battery)

    let percent = try battery.percentage(using: #require(appState.connectedDevice?.activeOCVArray))
  ...
  @Test func `absent battery reports no reading not zero percent`() {
    let appState = AppState()
  ...
  @Test func `missing battery reading reports no reading`() {
    let appState = AppState()
  ...
  @Test func `nil app state means not ready`() {
  ...
  @Test func `millivolts convert to percent through OCV curve`() {
    let battery = BatteryInfo(level: Self.seededLevel)
