...
struct SendAdvertIntentTests {
    ...
    @Test func reachMapsToFloodFlag() {
    ...
    @Test func reachRawValuesArePinnedShortcutIdentifiers() {
    ...
    @Test func successDialogIsReachSpecificAndHonest() {
    ...
    @Test func mapToIntentErrorRoutesEveryCase() {
    ...
    @Test func generalErrorMappingNeverLeaksRawErrors() {
    ...
    @Test func gpsGateRespectsThePerDevicePreference() {
        let appState = AppState()
        let suite = UserDefaults(suiteName: "test.advert.gate.\(UUID().uuidString)")!
    ...
    @Test func gpsGateIsNilWithoutADevice() {
        let appState = AppState()
        let store = DevicePreferenceStore(userDefaults: UserDefaults(suiteName: "test.advert.gate.\(UUID().uuidString)")!)
        #expect(appState.advertGPSSource(device: nil, store: store) == nil)
    }

    // MARK: - Disconnected guard

    @Test func sendSelfAdvertWithoutServicesThrowsNotConnected() async {
        let appState = AppState()
        do {
