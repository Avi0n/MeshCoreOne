...
/// the 12 shipped `.lproj` bundles surfaces as the raw key in that language. These
/// tests pin every metadata key the intents reference and confirm each resolves
/// to real copy in all 12 locales, never the raw-key fallback.
struct IntentMetadataLocalizationTests {
  ...
  /// The 12 shipped locales. A key missing from any one falls back to the raw
