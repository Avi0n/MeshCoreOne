...
struct WiFiTransportTests {
  @Test
  func `Initial state is disconnected`() async {
  ...
  @Test
  func `Advertises pipelined reads without a Write-Without-Response characteristic`() async {
    let transport = WiFiTransport()
  ...
  @Test
  func `Connect without configuration throws notConfigured`() async {
    let transport = WiFiTransport()
  ...
  @Test
  func `Connection to invalid host fails`() async {
    let transport = WiFiTransport()
  ...
  @Test
  func `Send without connection throws notConnected`() async {
    let transport = WiFiTransport()
  ...
  @Test
  func `Disconnect when not connected is safe`() async {
    let transport = WiFiTransport()
  ...
  @Test
  func `Disconnection handler not called on user-initiated disconnect`() async {
    let transport = WiFiTransport()
    let callTracker = CallTracker()
  ...
  @Test
  func `Disconnection handler not called on initial connect failure`() async {
    let transport = WiFiTransport()
    let callTracker = CallTracker()
  ...
  @Test
  func `connect() is idempotent — second call does not create new TCP connection`() async throws {
  ...
  @Test
  func `Peer close finishes the stream and fires the disconnection handler`() async throws {
    let acceptedConnections = OSAllocatedUnfairLock<[NWConnection]>(initialState: [])
  ...
  @Test
  func `Cancelling connect() does not leave it parked`() async throws {
    let transport = WiFiTransport()
