...
struct MeshCoreSessionGetChannelsTests {
    ...
    @Test("each requested index lands in its own slot and unrequested indexes are ignored")
    func requestedIndexesLandInCorrectSlots() async throws {
    ...
    @Test("dropped writes surface as missing indexes after the idle timeout")
    func droppedWritesSurfaceAsMissing() async throws {
        let transport = MockTransport()
        await transport.setSupportsWriteWithoutResponse(true)
        let session = MeshCoreSession(
            transport: transport,
    ...
    @Test("no more than the window is outstanding before the first response, then refills")
    func windowBoundsOutstandingWritesAndRefills() async throws {
        let transport = MockTransport()
        await transport.setSupportsWriteWithoutResponse(true)
        let session = MeshCoreSession(
            transport: transport,
    ...
    @Test("a cancelled getChannels does not leak a late channelInfo into a same-type successor")
    func cancelledPipelineDoesNotCorruptNextCommand() async throws {
        let transport = MockTransport()
        await transport.setSupportsWriteWithoutResponse(true)
        let session = MeshCoreSession(
            transport: transport,
    ...
    @Test("falls back to serial reads when the transport lacks write-without-response")
    func fallsBackToSerialWhenCapabilityAbsent() async throws {
