...
struct ContactNameDecodingTests {
    ...
    @Test("Flag emoji split at the name boundary decodes to a readable prefix")
    func splitFlagDecodesToPrefix() throws {
    ...
    @Test("Lone regional indicator is kept when only half a flag survives")
    func loneRegionalIndicatorKept() throws {
    ...
    @Test("A flag name that fits the buffer decodes unchanged")
    func validFlagNameUnchanged() throws {
        let nameField = Data("Team \u{1F1FA}\u{1F1F8}".utf8) + Data([0x00]) // "Team 🇺🇸"
        let contact = try #require(Parsers.parseContactData(contactBody(nameField: nameField)))
        #expect(contact.advertisedName == "Team \u{1F1FA}\u{1F1F8}")
    }

    @Test("Plain ASCII name still decodes")
    func asciiNameDecodes() throws {
