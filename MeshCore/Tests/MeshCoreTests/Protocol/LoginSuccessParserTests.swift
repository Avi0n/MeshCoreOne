...
struct LoginSuccessParserTests {
    ...
    private static func extractLogin(_ event: MeshEvent) -> LoginInfo? {
        if case .loginSuccess(let info) = event { return info }
        return nil
    }
    ...
    @Test("C++ admin login (byte0=1, ACL=0x03) parses as admin on both repeater and room-server")
    func cppAdminParsesAsAdmin() {
    ...
    @Test("C++ repeater guest login (byte0=0, ACL=0x00) parses as guest")
    func cppRepeaterGuestParsesAsGuest() {
    ...
    @Test("C++ repeater read-write login (byte0=0, ACL=0x02) parses as read-write, not admin")
    func cppRepeaterReadWriteParsesAsReadWrite() {
    ...
    @Test("C++ room-server guest login (byte0=2, ACL=0x00) parses as non-admin guest")
    func roomServerGuestParsesAsNonAdminGuest() {
    ...
    @Test("C++ room-server read-only login (byte0=0, ACL=0x01) parses as non-posting guest")
    func roomServerReadOnlyParsesAsGuest() {
    ...
    @Test("pyMC admin login (ACL=0x02) parses as admin")
    func pyMCAdminParsesAsAdmin() {
    ...
    @Test("pyMC guest login (ACL=0x01) parses as non-admin guest")
    func pyMCGuestParsesAsNonAdmin() {
        let payload = Self.extendedPayload(isAdminByte: 0, aclPermissions: 0x01)
        let info = Self.extractLogin(Parsers.LoginSuccess.parse(payload))
        #expect(info?.isAdmin == false)
    ...
    @Test("Legacy 7-byte payload with companion-radio hardcoded 0 parses as non-admin")
    func legacyHardcodedZeroParsesAsNonAdmin() {
    ...
    @Test("Payload shorter than 7 bytes returns zero LoginInfo fallback")
    func shortPayloadReturnsZeroLoginInfo() {
    ...
    @Test("Extended-format pubkey prefix is extracted from bytes 1..<7")
    func extendedPubkeyPrefixPropagates() {
