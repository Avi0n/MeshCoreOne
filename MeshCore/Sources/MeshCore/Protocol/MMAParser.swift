...
enum MMAParser {
  ...
  private static func decodeToDouble(type: LPPSensorType, data: Data) -> Double {
    ...
    case .humidity:
      Double(data[0]) * 0.5
    case .temperature:
      Double(readInt16BE(data)) / 10.0
    case .barometer:
      Double(readUInt16BE(data)) / 10.0
    case .voltage:
      Double(readUInt16BE(data)) / 100.0
    case .current:
      Double(readUInt16BE(data)) / 1000.0
    case .illuminance, .concentration, .power, .direction:
      Double(readUInt16BE(data))
    case .altitude:
      Double(readInt16BE(data))
    case .load:
      Double(readInt24BE(data)) / 1000.0
    case .analogInput, .analogOutput:
      Double(readInt16BE(data)) / 100.0
    case .genericSensor:
      Double(readUInt32BE(data))
    case .frequency:
      Double(readUInt32BE(data))
    case .distance, .energy:
      Double(readUInt32BE(data)) / 1000.0
    case .unixTime:
      Double(readUInt32BE(data))
    case .accelerometer, .gyrometer, .colour, .gps:
      // Complex types - return first component only for MMA
      Double(readInt16BE(data)) / (type == .accelerometer ? 1000.0 : 100.0)
    }
  }
  ...
  private static func readInt24BE(_ data: Data, offset: Int = 0) -> Int32 {
    ...
    var value = Int32(data[offset]) << 16
