...
private enum ChannelPipelineOutcome {
...
public extension MeshCoreSession {
  ...
  func getChannel(index: UInt8) async throws -> ChannelInfo {
    try await sendAndWait(PacketBuilder.getChannel(index: index)) { event in
      if case let .channelInfo(info) = event, info.index == index { return info }
      return nil
    }
  }
  ...
  func getChannels(indices: [UInt8]) async throws -> (received: [ChannelInfo], missing: [UInt8]) {
    ...
    guard await transport.supportsPipelinedReads else {
      ...
      for index in indices {
        try await received.append(getChannel(index: index))
      }
      ...
      let received: [UInt8: ChannelInfo] = try await withThrowingTaskGroup(
        ...
        group.addTask {
          ...
          for await event in events {
            guard case let .channelInfo(info) = event,
        ...
        for try await outcome in group {
          if case let .collected(dict) = outcome {
            collected = dict
            group.cancelAll()
            break
          }
  ...
  /// - Throws: ``MeshCoreError/timeout`` or ``MeshCoreError/deviceError(code:)`` on failure.
  func setChannel(index: UInt8, name: String, secret: ChannelSecret = .deriveFromName) async throws {
